...
opts.expDir = 'data/fcn8s-voc12' ;
opts.dataDir = 'data/voc12' ;
opts.modelPath = 'data/fcn8s-voc12/pascal-fcn8s-dag.mat' ;
...
for i = 1:numel(val)
  ...
  % Accumulate errors
  %ok: pick the ground-truth segmented result
  ok = lb > 0 ;
  %lb(ok): generate a column array which is formed by true-position in ok
  %confusion: confuse the non-zero area in a 21*21 figure, 21 since there
  %are 20 classes in all
