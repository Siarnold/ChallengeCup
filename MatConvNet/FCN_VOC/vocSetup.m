...
%vocSetup downloads and organizes an imdb according to input settings
...
imdb.images.name = {} ;
imdb.images.set = [] ;
% Map is a MATLAB class/container, which goes by key
...
%simplify the posi of directory
switch opts.edition
  case '11'
    % * means move all directories and files in that directory
...
%there are all 20 train textfiles corresponding to the imdb.classes.names
for ci = 1:length(imdb.classes.name)
  className = imdb.classes.name{ci} ;%'bicycle'
  annoPath = fullfile(opts.dataDir, 'ImageSets', 'Main', ...
    [className '_' setName '.txt']) ;
  fprintf('%s: reading %s\n', mfilename, annoPath) ;
  %read from this textfile
  [names,labels] = textread(annoPath, '%s %f') ;
  %for this one piece of image, add it to imdb, and set up a MAP from
  %image_filename to its imdb.images.id
  for i=1:length(names)
    if ~index.isKey(names{i})
      ...
      %add classification as a field
      imdb.images.classification(j) = true ;
    else
      j = index(names{i}) ;
    end
    %add to imdb.classes.images when label > 0
